...
/* Class : KWA134_TC03_GoBackFunctionality -- Validate on Click on Go Back, a=user should be redirected to Confirmation screen
 *  Author : Sayed Zeeshan Haidar
 * (C) Xavient Information System 2017
 */

public class Sprint3_KWA134_TC03_GoBackFunctionality extends GenericClass {
	...
	public void GoBackFunctionality() {
		...
		try {
			...
			if (Utility.touchEvent(Utility
					.getLocator(KioskLocatorsMapper.goBackButtonCC))) {
				...
				Log.info("Go back button functionality is not working fine");
			}
			
			//verify user navigated to confirmation screen
			if (Utility.currentURL().endsWith("card-send/confirmation")) {
				Reports.pass("As Expected user navigated  -----> " + Utility.currentURL() + " On click of Go Back button");
				Log.info("As Expected user navigated  -----> " + Utility.currentURL() + " On click of Back button");
			} else {
				imagePath = Utility.getfailScreenshot();
				Reports.attachfailScreenshot("Execption Occured", imagePath);
				Reports.fail("Go back button functionality is not working fine");
