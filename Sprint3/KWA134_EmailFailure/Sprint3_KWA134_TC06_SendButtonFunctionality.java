...
/* Class : Sprint3_KWA134_TC06_SendButtonFunctionality -- Validate Send button is Placeholder
 *  Author : Sayed Zeeshan Haidar
 * (C) Xavient Information System 2017
 */

public class Sprint3_KWA134_TC06_SendButtonFunctionality extends GenericClass {
	...
	public void SendButtonFunctionality() {
		...
		try {
			...
			// Feed Data on Second Screen of Personalization screen and move to
			// Confirmation screen
			...
			if (Utility.touchEvent(Utility
					.getLocator(KioskLocatorsMapper.buyButtonScanPromptScreen))) {
				Reports.pass("User tap on Send button");
				Log.info("User tap on Send button");
				if (Utility.currentURL().endsWith("card-send/email-failure")) {
					Reports.pass("As expected, Send button is placeholder and user remain stay on Email Failure Screen");
					Log.info("As expected, Send button is placeholder and user remain stay on Email Failure Screen");
				} else {
					imagePath = Utility.getfailScreenshot();
					Reports.attachfailScreenshot("Execption Occured", imagePath);
					Reports.fail("Send button is enable, which is wrong behavior");
				}
			} else {
				imagePath = Utility.getfailScreenshot();
				Reports.attachfailScreenshot("Execption Occured", imagePath);
				Reports.fail("Send button is enable, which is wrong behavior");
				Log.info("Send button is enable, which is wrong behavior");
			}
