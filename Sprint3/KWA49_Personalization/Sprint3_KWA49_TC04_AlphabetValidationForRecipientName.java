...
/* Class : Sprint3_KWA49_TC04_RecipentNameAlphabetValidation -- Verify 'Recipient's Name' field by entering alphabets only
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC04_AlphabetValidationForRecipientName extends GenericClass{
	...
	@Test
	public void alphabetsValidation(){
		...
		try{
			...
			Thread.sleep(2000);
		/*	if(Utility.getLocator(KioskLocatorsMapper.recipientName).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("RNameAlphabet"))){
				Reports.pass("User Able to enter the alphabets in Recipient Name Field");*/
			...
			/*}else{
				imagePath = Utility.getfailScreenshot();
				Reports.attachfailScreenshot("", imagePath);
				Reports.fail("User is unable to enter the alphabets in Recipient Name");
				}		*/
