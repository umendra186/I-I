...
/* Class : Sprint3_KWA49_TC05_NumberValidationForRecipentName -- Verify 'Recipient's Name' field by entering number only
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC05_NumberValidationForRecipientName extends GenericClass{
	...
	public void NumberValidation(){
		...
		try{
			...
			Utility.getLocator(KioskLocatorsMapper.recipientName).sendKeys(ExcelUtils.readDataForPersonalization().get("RNameNumber"));
			Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
			Thread.sleep(2000);
			/*if(Utility.getLocator(KioskLocatorsMapper.recipientName).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("RNameNumber"))){
				Reports.pass("User Able to enter the Number in Recipient Name Field");*/
			...
			/*}else{
				imagePath = Utility.getfailScreenshot();
				Reports.attachfailScreenshot("", imagePath);
				Reports.fail("User is unable to enter the Number in Recipient Name");
				}		*/
