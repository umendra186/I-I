...
/* Class : Sprint3_KWA49_TC06_AlphanumericValidationForRecipentName -- Verify 'Recipient's Name' field by entering alphanumeric value
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC06_AlphanumericValidationForRecipientName extends GenericClass{
	...
	public void alphanumericValidation(){
		...
		try{
			...
			System.out.println("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("RNameAlphanumeric"));
			Utility.getLocator(KioskLocatorsMapper.recipientName).sendKeys(ExcelUtils.readDataForPersonalization().get("RNameAlphanumeric"));
			Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
			Thread.sleep(2000);
			/*if(Utility.getLocator(KioskLocatorsMapper.recipientName).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("RNameAlphanumeric"))){
				Reports.pass("User Able to enter the Alphanumeric in Recipient Name Field");*/
				if(Utility.getLocator(KioskLocatorsMapper.recipientEmailErrorMsg).getText().equals("Please enter valid email") && Utility.getLocator(KioskLocatorsMapper.recipientEmailErrorMsg).getText().equals("Please enter valid email") ){
				Reports.pass("As Expected,Error message is displayed for corresponding filed when user enter the value in only Recipient Name and click on next button");
				}else{
			...
			/*}else{
				imagePath = Utility.getfailScreenshot();
				Reports.attachfailScreenshot("", imagePath);
				Reports.fail("User is unable to enter the Alphanumeric in Recipient Name");
				}		*/
