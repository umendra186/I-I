...
/* Class : Sprint3_KWA49_TC08_ValidEmailValidationForRecipentEmail -- Verify the valid input entry in 'Recipient's Email' field
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC08_ValidEmailValidationForRecipientEmail extends GenericClass{
	...
	@Test
	public void validEmailValidationForRecipentEmail(){
		...
		try{
			...
			Log.info("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("REmailValidEmail"));
			...
			Thread.sleep(2000);
			/*if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("REmailValidEmail"))){
				Reports.pass("User Able to enter the valid email in Recipient Email field ");*/
				if(Utility.getLocator(KioskLocatorsMapper.senderEmailErrorMsg).getText().equals("Please enter valid email") && Utility.getLocator(KioskLocatorsMapper.recipientNameErrorMsg).getText().equals("Please enter valid name") ){
			...
			/*}else{
				imagePath = Utility.getfailScreenshot();
				Reports.attachfailScreenshot("", imagePath);
				Reports.fail("User is unable to enter the valid Email in Recipient Email");
				}*/
