...
/* Class : Sprint3_KWA49_TC09_AtTheRateSignValidationForRecipentEmail -- Verify 'Recipient's Email' field when  @ sign is missing
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC09_AtTheRateSignValidationForRecipientEmail extends GenericClass{
	...
	@Test
	public void atTheRateSignValidationForRecipentEmail(){
		...
		try{
			...
			Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).sendKeys(ExcelUtils.readDataForPersonalization().get("Remail@Missing"));
			Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
			Thread.sleep(2000);
			/*if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("Remail@Missing"))){
