...
/* Class : Sprint3_KWA49_TC10_AtTheRateSignAndDomainNameValidationForRecipentEmail --Verify 'Recipient's Email' field when @ sign and domain is missing
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC10_AtTheRateSignAndDomainNameValidationForRecipientEmail extends GenericClass{
	...
	@Test
	public void atTheRateSignAndDomainNameValidationForRecipentEmail(){
		...
		try{
			...
			Log.info("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("REmail@andDomainMissing"));
			Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).sendKeys(ExcelUtils.readDataForPersonalization().get("REmail@andDomainMissing"));
			Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
			Thread.sleep(2000);
			/*if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("REmail@andDomainMissing"))){
