...
/* Class : Sprint3_KWA49_TC12_RecipentEmailValidationWhenUserNameMissingFromEmail --Verify 'Recipient's Email' field when  username is missing 
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC12_RecipientEmailValidationWhenUserNameMissingFromEmail extends GenericClass{
	...
	@Test
	public void recipentEmailValidationWhenRecipentNameMissing(){
		...
		try{
			...
			Log.info("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("REmailWhenUserNameMissing"));
			Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).sendKeys(ExcelUtils.readDataForPersonalization().get("REmailWhenUserNameMissing"));
			Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
			Thread.sleep(2000);
			/*if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("REmailWhenUserNameMissing"))){
