...
/* Class : Sprint3_KWA49_TC15_TrailingDotValidationForRecipientEmail --Verify 'Recipient's Email' field when enter Trailing  dot in address
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC15_TrailingDotValidationForRecipientEmail extends GenericClass{
		...
		@Test
		public void trailingDotValidationForRecipientEmail(){
			...
			try{
				...
				Log.info("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("REmailTrailingDot"));
				Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).sendKeys(ExcelUtils.readDataForPersonalization().get("REmailTrailingDot"));
				Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
				Thread.sleep(2000);
				/*if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("REmailTrailingDot"))){
