...
/* Class : Sprint3_KWA49_TC16_MultipleDotValidationForRecipientEmail --Verify 'Recipient's Email' field when enter Multiple dots 
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC16_MultipleDotValidationForRecipientEmail extends GenericClass{
		...
		@Test
		public void multipleDotValidationForRecipientEmail(){
			...
			try{
				...
				Log.info("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("REmailMultipleDot"));
				Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).sendKeys(ExcelUtils.readDataForPersonalization().get("REmailMultipleDot"));
				Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
				Thread.sleep(2000);
			/*	if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("REmailMultipleDot"))){
