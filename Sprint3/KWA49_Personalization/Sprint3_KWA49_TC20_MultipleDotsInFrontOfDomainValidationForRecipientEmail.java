...
/* Class : Sprint3_KWA49_TC20_MultipleDotsInFrontOfDomainValidationForRecipientEmail --Verify 'Recipient's Email' field when enter Multiple dot in the domain portion
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC20_MultipleDotsInFrontOfDomainValidationForRecipientEmail extends GenericClass{
		...
		@Test
		public void mltipleDotsInFrontOfDomainValidationForRecipientEmail(){
			...
			try{
				...
				Log.info("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("REmailMultipleDotInFrontDomain"));
				Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).sendKeys(ExcelUtils.readDataForPersonalization().get("REmailMultipleDotInFrontDomain"));
				Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
				Thread.sleep(2000);
				/*if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("REmailValidEmail1"))){
					Reports.pass("User Able to enter the valid email in Recipient Email field ");*/
			if (Utility.getLocator(KioskLocatorsMapper.recipientEmailErrorMsg).getText().equals("Please enter valid email")
				...
				/*}else{
					imagePath = Utility.getfailScreenshot();
					Reports.attachfailScreenshot("", imagePath);
					Reports.fail("User is unable to enter th valid Email in Recipient Email");
					}*/
