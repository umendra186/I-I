...
/* Class : Sprint3_KWA49_TC24_GarbageDataValidationForSenderEmail --This test case is to verify the validation on 'Sender's Email' field when user enter email which have garbage data 
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC24_GarbageDataValidationForSenderEmail extends GenericClass{
	...
	@Test
	public void garbageDataValidationForSenderEmail(){
		...
		try{
			...
			Log.info("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("REmailGarbageValue"));
			Utility.getLocator(KioskLocatorsMapper.senderEmailInputBox).sendKeys(ExcelUtils.readDataForPersonalization().get("REmailGarbageValue"));
			Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
			Thread.sleep(2000);
			/*if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("REmailGarbageValue"))){
