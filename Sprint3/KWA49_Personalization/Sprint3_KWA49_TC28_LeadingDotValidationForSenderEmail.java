...
/* Class : Sprint3_KWA49_TC27_LeadingDotValidationForSenderEmail --This test case is to verify the validation on 'Sender's Email' field when user enter email which contain Leading dot in address 
*  * Author : Umendra Singh Tomar
* (C) Xavient Information System 2018
* 
*/
public class Sprint3_KWA49_TC28_LeadingDotValidationForSenderEmail extends GenericClass{
		...
		@Test
		public void leadingDotValidationForSenderEmail(){
			...
			try{
				...
				Log.info("Excel Value--->"+ExcelUtils.readDataForPersonalization().get("REmailLeadingDot"));
				Utility.getLocator(KioskLocatorsMapper.senderEmailInputBox).sendKeys(ExcelUtils.readDataForPersonalization().get("REmailLeadingDot"));
				Utility.touchEvent(Utility.getLocator(KioskLocatorsMapper.nextButton));
				Thread.sleep(2000);
				/*if(Utility.getLocator(KioskLocatorsMapper.recipientEmailInputBox).getAttribute("ng-reflect-model").equals(ExcelUtils.readDataForPersonalization().get("REmailLeadingDot"))){
